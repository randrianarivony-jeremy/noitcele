  ...
  return (
    <Box id="group-B" minH={height}>
      ...
      <HStack
        ...
        spacing={[2, 3, 4]}
      >
        {groupB.map(({ name, nb, picture }) => (
          <Link
            rounded={'lg'}
            padding={2}
            align={'flex-start'}
            cursor={'pointer'}
            boxShadow={semifinalists.B.nb === nb ? '0 0 10px red' : 'lg'}
            _hover={{ boxShadow: semifinalists.B.nb !== nb && '2xl' }}
            href="#group-C"
            key={nb}
            maxW={300}
            ...
            width={['100%', '45%']}
            onClick={() =>
              setSemifinalists({ ...semifinalists, B: { name, picture, nb } })
            }
          >
